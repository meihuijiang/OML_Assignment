...
class SC4RUX:
    ...
    def chvatal(self, c, A):
        ...
        def chvatal_heuristic(samples: List[int], rules: List[int], impurity: List[float]) -> List[int]:
          num_samples = len(samples)
          num_rules = len(rules)
          
          # Create a matrix that represents whether a rule covers a sample
          covering_matrix = np.zeros((num_rules, num_samples))
          for i in range(num_samples):
            for j in range(num_rules):
              if samples[i] in rules[m]:
                covering_matrix[m][n] = 1
                
                # Create a dictionary to store the cost of each rule
                rule_cost = dict()
                for m in range(num_rules):
                  rule_cost[m] = 1 + impurity[m]
                  
                  # Create a list to store the selected rules
                  selected_rules = []
                  
                  # Continue selecting rules until all samples are covered
                  while np.sum(covering_matrix) > 0:
                    # Find the rule with the highest ratio of cost to uncovered samples
                    max_ratio = -1
                    max_rule = -1
                    for m in range(num_rules):
                      if m not in selected_rules:
                        rule_coverage = covering_matrix[m]
                        uncovered_samples = np.sum(rule_coverage == 1)
                        ratio = rule_cost[m] / uncovered_samples
                        if ratio > max_ratio:
                          max_ratio = ratio
                          max_rule = m
                          
                          # Add the selected rule to the list and update the covering matrix
                          selected_rules.append(max_rule)
                          covering_matrix[max_rule] = 0
